...
class KnowledgatorBiomedScript(Script):
    ...
    bridge drops/normalizes them instead of crashing. The observed vocabulary is 32 labels, not the 24
    documented on the HF card; LABEL_MAP covers the 21 canonical classes that have honest biolink
    targets (LANGUAGE, REGULATION OR LAW, MONEY have none) plus 7 plural/legacy variants (28 entries)
    and leaves the catch-alls (LANGUAGE, REGULATION OR LAW, MONEY, Unlabelled, INTELLECTUAL) unmapped
    so they surface as raw PascalCase tails.
    """

    NAME: ClassVar[str] = "KnowledgatorBiomedScript"

    # lowercase dataset classes -> biolink classes (resolve_mentions looks up raw_class.lower());
    # covers the 24 canonical labels plus the measured plural/legacy variants (PRODUCTS 350,
    # ORGANISMS 32, GENES 21, ORGANIZATIONS 9, FINDINGS/PHENOTYPES 3, DISORDERS 2 in a 901-row
    # sample); values are validated against tablassert Categories at import below
    ...
    def run(self: Self, values: ScriptValues) -> TrainingExample:
        ...
        labeled: list[ResolvedMention] = [
            ...
            for item, (_start, _end, label) in zip(resolved, spans, strict=True)
        ]
