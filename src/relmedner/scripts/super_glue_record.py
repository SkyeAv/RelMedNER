...
class SuperGlueRecordScript(Script):
    ...
    # validated at import by validate_category below
...
def validate_category(script: type[SuperGlueRecordScript]) -> None:
    """fail loudly at import when the shared span category is not a biolink class
    (mirror of families.validate_label_map for the single-constant case)"""
    if not ScriptUtils.is_biolink_category(script.CATEGORY):
        raise ValueError(f"{script.NAME} CATEGORY {script.CATEGORY!r} is not a biolink class")


validate_category(SuperGlueRecordScript)
