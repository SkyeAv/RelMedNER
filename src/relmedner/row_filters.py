...
def first_drop_reason(values: tuple[Any, ...], filters: RowFilters) -> str | None:
    ...
    max_text_len -> include_regex -> exclude_regex, or None when the row passes. match_on is NOT
    evaluated here: it is exact-value membership applied by the streams before this evaluator.

    The text every length/regex rule measures is joined_text(values) (see its docstring for the
    exact contribution rule)."""
    if filters.drop_empty and all(is_empty_value(value) for value in values):
        return "drop_empty"
    text: str | None = None
    if filters.min_text_len is not None or filters.max_text_len is not None:
        ...
        if filters.max_text_len is not None and len(text) > filters.max_text_len:
            return "max_text_len"
    if filters.include_regex is not None or filters.exclude_regex is not None:
        if text is None:
            text = joined_text(values)
