...
requires_live_hf: pytest.MarkDecorator = pytest.mark.skipif(
    ...
    reason="streams a real bigbio/osiris row from the hub; set RELMEDNER_LIVE_HF=1 (wenceslaus) to run",
)


@requires_live_hf
def test_a_live_streamed_osiris_row_dispatches_end_to_end() -> None:
    """live smoke over the first real row of the declared osiris_bigbio_kb parquet. Hand-built
    ...
    source, payload = next(
        ...
        if getattr(dataset, "file", None) == "osiris_bigbio_kb/train/0000.parquet"
    )
    ...
    assert Name == "bigbio/osiris"
    assert Task == ("script", "OsirisScript", ("entities",))

    Outputs, Example = Script.dispatch("OsirisScript", (tuple(Task[2]), Values))
    assert Outputs == ("entities",)
    assert isinstance(Example, TrainingExample)
    assert Example.populated() == {"entities"}
    # the first real document (12890863) carries gold gene + variant mentions; the entities
    # shape must fill, and every emitted surface must occur in the re-joined token text
    surfaces: list[str] = [mention for entity in Example.entities for mention in entity.mentions]
    assert surfaces, "the first real row must emit its gold gene/variant mentions"
    for surface in surfaces:
        assert surface in Example.text, f"mention {surface!r} does not occur in the text"
    assert {entity.label for entity in Example.entities} <= {"Gene", "SequenceVariant"}
