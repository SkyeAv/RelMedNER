...
from relmedner.scripts.super_glue_record import SuperGlueRecordScript, validate_category
...
def test_the_import_time_category_guard_rejects_a_non_biolink_class() -> None:
    ...
    validate_category is the module's import-time guard, exercised here on a stand-in subclass"""
    try:

        class BrokenScript(SuperGlueRecordScript):
            NAME: ClassVar = "BrokenRecordScript"
            CATEGORY: ClassVar = "NotAClass"

        with pytest.raises(ValueError, match="NotAClass"):
            validate_category(BrokenScript)
    finally:
        Script.REGISTRY.pop("BrokenRecordScript", None)
