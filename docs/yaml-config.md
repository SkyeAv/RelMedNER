...
| `sample_rate` | no | 1.0 | 0 < r <= 1 | fraction of this source's passing rows the run keeps at all: the mixing-ratio lever that also cuts compute (a 7M-row source at `sample_rate: 0.25` mines, dispatches, and dedups a quarter of it AND holds a quarter of the corpus share, which `weight` cannot do). Deterministic per row content (blake2b of the source key plus the row), so resumes and re-parallelized passes re-make the identical decision; runs AFTER the row filters, and sampled-out rows attribute to the `sample_rate` reason in the per-source quality line. Keyword-only, like `trust` and `filters`. When to prefer it over `weight`: docs/weighting.md |
| `read_shards` | no | 1 | integer >= 1 | how many parallel readers to split this source's read across. Each shard is one element of the pipeline's Create stage, so through the fan-out Reshuffle the read spreads over that many cluster tasks instead of serializing the whole dataset on one subtask -- the lever for the row-count outliers (a 1M-row source alongside 40 tiny ones). Sharding is transparent: local avro splits at whole-block granularity, hub sources split at the hub's file-shard granularity (clamped to the dataset's own shard count), the union of all shards' rows equals the unsharded row list, and `sample_rate` decisions stay per-row content so shard boundaries cannot change them. The per-source quality line carries a `[shard i/N]` suffix when sharded, and the zero-yield raise degrades to a per-shard warning there (a trailing file shard may legitimately hold zero passing rows; row counts for sharded sources are confirmed by the probe receipts). Only sources whose reader supports sharding accept a value over 1 -- anything else fails validation at parse time. Sizing rule: declare it on the row-count outliers, sized so total Create elements stay at or above total slots (`cluster.yaml`) -- e.g. 5 outliers at `read_shards: 4-8` lift ~79 datasets from 79 elements to 102 over an 80-slot cluster, and each outlier's read then spreads over its own 4-8 tasks instead of one |
